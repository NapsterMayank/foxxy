...
| **TRACK B** | **A SECOND WORKSTREAM EXISTS since 21 August — a new retrieval module, designed, nothing built.** Separate repository; only its documents live here. **It does not modify anything in this table**, and Foxxy's `retrieval` module is out of its scope permanently (D-387). Start at `docs/superpowers/plans/2026-08-21-rag-build-guide.md`. Blocked on one client decision: named pilot scope. Everything else in this file is Track A |

---

## 2. THE NEXT ACTION

> **Two workstreams since 21 August 2026.**
>
> **Track A — the Foxxy product.** Everything in this file. Next action below.
>
> **Track B — a new, separate retrieval module. CODE NOW LIVES ELSEWHERE:**
> `D:\personal\agts-retrieval` (D-400). **Phases 0-2 built, holdout-validated
> and persisted, 30 August** — parse, compose, chunk, embed, rank, a working
> abstention gate, cited evidence packs, release manifests and traces. On 38
> unseen cases: **8/8 unanswerable refused, 27/30 answered**, citation
> completeness 96.3% against §14's 95% bar, 0 lineage failures. Two real NCERT
> chapters parsed (Science ch1, Maths ch4), 138 tests, its own Postgres
> container. **Nothing is releasable**: every number is under an evaluation
> licence over QUARANTINED content. Read `agts-retrieval/EVALUATION_LEDGER.md`
> and its R-001..R-033. **Build order is the client's AI-native guide** (22 August, 72-hour
> execution model) — `plans/2026-08-21-rag-build-guide.md` is **SUPERSEDED**
> (D-398) and the v2 spec is now authority 3. Decisions:
> `docs/03-DECISION-LOG.md` **D-387..D-400**. **Two blockers, both client-side:
> named pilot scope, and signed rights records per source — rights are blocking
> again because the client's own §5 says verbal assurance is not a rights
> record** (D-399). Open questions in
> `agts-retrieval/docs/03-open-questions.md`.
> **Track B does not modify Foxxy's `retrieval` module and never will**
> (D-387); it is now a post-design comparator only. This file stays about
> Track A.
...
| `docs/superpowers/plans/2026-08-21-rag-build-guide.md` | **TRACK B — START HERE.** Phase-by-phase build order in plain language, with checkboxes and per-phase guardrails. Read before the spec |
| `docs/superpowers/plans/2026-08-21-phase-0-howto.md` | **TRACK B — how to actually build Phase 0.** Day-by-day with code: docker + schema, docling parse, chunking rules, Voyage embed, scoring script, the 50-question answer key, model bake-off |
| `docs/superpowers/specs/2026-08-20-ncert-rag-design.md` | **TRACK B** — design record for the new retrieval module (Pedagogical Evidence Retrieval Fabric). Schemas, contracts, thresholds. Separate system; changes nothing here. Implements §5-6 of the client's AGTS dossier |
| `D:\Downloads\Alfanumrik_Clean_Sheet_RAG_Architecture (1).docx` | **client-supplied**, not in the repo — AGTS v1.0, 21 Aug. The parent architecture Track B implements a slice of. AGTS wins on policy; our spec wins on implementation detail |
...
| 21 Aug 2026 | **TRACK B opened — a new retrieval module, designed, nothing built.** No code in this repository changed. Read Foxxy's `retrieval` module end to end first and found the diagnosis the design answers: 3,892 lines of hybrid dense+sparse with RRF, dedup, a calibrated abstention floor and a per-turn trace — and `eval/retrieval/golden/types.ts` is `{ query, grade, subject, note }` with **no gold chunk id**, so retrieval correctness has never been computed here. The client then supplied the **AGTS dossier** (v1.0, 21 Aug, 47 references), which our independently-written design matched at the retrieval layer and was a strict subset of in scope. Merged both into spec v2: AGTS as parent, ours as its §5-6. **Four corrections came from the dossier** — rights inverted to quarantine-by-default with NCERT explicitly not exempt [R45]; policy filters moved before scoring; a security threat model added where v1 had none; the curriculum graph promoted to Phase 1. **One thing kept against it**: a post-rerank pack-recall gate, because AGTS G1 stops at candidate recall and nothing measures what reaches the renderer. Estimate moved 10-14 → 17-19 weeks, and the increase is all AGTS requirements v1 did not carry. D-387..D-396 | unchanged — no code written |
| 22 Aug 2026 | **Track B made usable, and the rights blocker withdrawn.** The spec reads as a design record and the client could not work from it, so two readable documents were written beside it: `plans/2026-08-21-rag-build-guide.md` (phase-by-phase, 44 checkboxes, a boxed guardrail per phase) and `plans/2026-08-21-phase-0-howto.md` (day-by-day with runnable code — docker, schema, docling parse, the four chunking rules, Voyage embed, the scoring function, the 50-question answer key, the model bake-off). All three now cross-link, and each states which one wins on a disagreement: **spec wins on precision, guides win on readability.** **The client states rights to NCERT and the other books are already held**, so D-392's schedule impact is withdrawn while its mechanism survives — sources start APPROVED with a basis recorded rather than blocking (D-397, which amends D-392 and says so in D-392's own entry). **Named pilot scope is now the only non-engineering blocker on Phase 1.** Phase 0 chapter recommended: grade 10 Science "Light: Reflection and Refraction" — prose, equations, ray diagrams and tables, and Foxxy holds it as ch 9 for a direct comparison. D-397 | unchanged — no code written |
...
7. **If the session touched TRACK B**, update its own documents too — steps 1-5 above describe Track A only, and a Track B session that follows them leaves its own plan stale. Track B's files are `docs/superpowers/plans/2026-08-21-rag-build-guide.md` (tick the checkboxes), `docs/superpowers/plans/2026-08-21-phase-0-howto.md`, `docs/superpowers/specs/2026-08-20-ncert-rag-design.md`, and `.dual-graph/context-store.json`. Append Track B decisions to the same decision log, continuing the D-number sequence. **A session-log row here is still required** — one file holds the whole history, and a Track B row simply says "no code in this repository changed".
