...
| Frontend | **WIRED, 12 August.** It makes real network calls for the first time. Four of step 0's five foundation gaps are closed — session strategy, the error-code table, the Foxy streaming client (all 7 cases in plan §7 are tests) and the token scales — plus build-order step 6, the typed client and providers. Backend contracts are GENERATED into `src/lib/api/generated/` with a drift test, because `frontend/Dockerfile` copies `frontend/` alone and a direct cross-package import cannot exist in the image. **Tests 10 → 57.** What is left of step 0 is the CI gates (§10.7), none of which has been proven to fail on a deliberate violation |
| Marketing site | **scaffolded** - 32 files under `website/`, committed. Per `06-FRONTEND-SEPARATION-PLAN.md` |
| Tests | **3,173 backend passing**, 185 files, plus **57 frontend** (was 10 — see §5). Three audit waves have added 656 between them. **Every guard, threshold and validation in the codebase has been broken deliberately and confirmed to turn a named test red** - see D-214 |
...
| Frontend build-order step 0 — **4 of 5 done 12 August**; the CI gates (§10.7) remain | 1 |
...
Build order from `docs/02-FRONTEND-IMPLEMENTATION-PLAN.md` section 11. **Step 0 is four-fifths closed — 12 August 2026.** The frontend makes real network calls for the first time.

| Step | Item | Days |
|---|---|---|
| 🟡 0 | **Foundation gaps — 4 of 5 DONE.** Auth/session strategy · error-code table · streaming client · token values are built and tested. **CI gates (10.7) are not** | 1 |
| 🟡 1-4 | Tooling and tokens DONE (scales are CLOSED, not extended, and lint enforces it). `ui` primitives · `patterns` · `AppShell` remain | 6 |
| ⬜ 5 | i18n scaffold and both dictionaries · the user-facing-string lint rule | 2 |
...
**Frontend tests: 10 → 57.** Type-check, lint and the whole suite are green.
...
| 30 | ✅ **CLOSED except the CI gates — 12 August.** Four of the five foundation gaps are built and tested: session strategy, the error-code table, the streaming client and the token scales. See §5. **What remains is 10.7 — bundle budgets, Lighthouse, visual regression, coverage floors — and no gate has been proven to fail on a deliberate violation yet**, which is the bar the backend's eight gates met | frontend | 1 d |
...
| 32 | **Frontend coverage is unmeasurable** - `@vitest/coverage-v8` is not installed, so `--coverage` fails outright. Section 10.5's floors are not merely unmet, they cannot be evaluated | frontend | 0.25 d |
...
| 34 | **Two frontend shapes already disagree with the backend contract.** Onboarding submits `english`/`hindi`; the contract accepts `en`/`hi`. Grades are hardcoded 6-10, matching `PILOT_GRADES` rather than the `GRADES` the plan's test names. Latent - nothing posts yet | frontend | 0.25 d |
