/*
 * THE SAME TRACE SCREEN, REACHED FROM A TURN INSTEAD OF FROM A TRACE ID.
 *
 * One line differs: which endpoint it reads. Everything about what a trace
 * MEANS - that a non-zero fabricated-citation count is a grounding failure,
 * that `present: false` and `length: 0` are different failures, that the text
 * is behind Reveal - lives in the component below and is shared, because two
 * renderings of one row is how a field ends up redacted on one route and raw on
 * the other.
 */
'use client';
...
export default function FoxyTraceScreen({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error, loading } = useAdminData(adminPaths.foxyTraceByMessage(id), adminTraceResponseSchema);
